...
// ---- Patient details ----
@Field String firstName   = 'MargeA'
@Field String lastName    = 'SimpsonA'
@Field String patientAge  = '50yo'
@Field String dob         = '07/10/1976'
...
// STEP 6: Select the (same) appointment date, handling month rollover
KeywordUtil.logInfo('Step 6: Selecting reschedule appointment date')
selectAppointmentDate(tomorrowDay, tomorrowFullDate)

// STEP 7: Select the new appointment time
KeywordUtil.logInfo('Step 7: Selecting reschedule appointment time')
selectAppointmentTime(rescheduleApptTime)

// STEP 8-10: Enter reschedule reason free-text, choose Self Pay, finish booking
KeywordUtil.logInfo('Step 8-10: Entering reschedule reason, selecting payment option, and finishing booking')
enterReasonAndFinishBooking(rescheduleReasonInput)

// STEP 11: Verify the reschedule confirmation screen
KeywordUtil.logInfo('Step 11: Verifying reschedule confirmation screen')
verifyConfirmationScreen(ptNameLabel, [
    (findTestObject('Appointment Booking/Reschedule Appt/p_Your appointment has been rescheduled')) : expectedRescheduleMsg,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Name_ Marge Simpson'))                    : expectedName,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Location_ Katalon Location'))             : expectedLocation,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Provider_ Katalon Provider (OD)'))        : expectedProvider,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Reason_ Katalon Reason'))                 : expectedReason,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Date_ 08_11_2026', ['date': tomorrowFullDate]))       : expectedDate,
    (findTestObject('Appointment Booking/Reschedule Appt/p_Time_ 04_00 PM', ['time': rescheduleApptTime]))       : expectedRescheduledTime,
])

KeywordUtil.logInfo('Part 3 complete: Chat Bot Appointment Reschedule verified successfully')
KeywordUtil.markPassed('Chat Bot Appointment Reschedule test case passed successfully')

/* =============================================================================
 * PART 4: VERIFY THE RESCHEDULED APPOINTMENT IN MAXIMEYES
 * ========================================================================== */

KeywordUtil.logInfo('Switching back to the MaximEyes window')
WebUI.switchToDefaultContent()
WebUI.delay(2)
WebUI.switchToWindowIndex(1)

// STEP 1-2: Search for the patient and verify the rescheduled appointment slot
KeywordUtil.logInfo('Step 1-2: Searching for patient and verifying rescheduled appointment slot')
searchPatientAndVerifyAppointment(firstName, lastName, expectedRescheduleDateTimeReason)

// STEP 3: Verify the rescheduled appointment on the scheduler/calendar view
KeywordUtil.logInfo('Step 3: Verifying rescheduled appointment on the scheduler view')
verifyAppointmentOnScheduler(expectedSpanText)

/* =============================================================================
 * PART 5: CANCEL THE APPOINTMENT (OFFICE REQUEST)
 * ========================================================================== */

// STEP 1: Open the appointment actions dropdown
...
KeywordUtil.logInfo('Step 1: Appointment Actions dropdown opened successfully')

// STEP 2: Select "Cancel Appt (Office Request)"
...
KeywordUtil.logInfo('Step 2: "Cancel Appt (Office Request)" selected successfully')

// STEP 3: Navigate to the OA / settings page
...
KeywordUtil.logInfo('Step 3: OA page opened successfully')

WebUI.comment('TEST COMPLETE: All appointment booking, reschedule, and cancellation verifications for patient ' +
        "'${firstName} ${lastName}' passed")
KeywordUtil.logInfo('Test complete: booking, reschedule, and cancellation flows all verified successfully')
