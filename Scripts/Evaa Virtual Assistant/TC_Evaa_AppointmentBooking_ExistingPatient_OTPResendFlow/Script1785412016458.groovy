...
// STEP 6: Enter OTP  >> Need add these steps once OTP issue is resolved

//Verify Otp received on mail and mobile
WebUI.delay(5)
String otp1 = CustomKeywords.'utils.GmailOTPReaderChatBot.getOTP'(
...
String otp2 = CustomKeywords.'utils.GmailOTPReaderChatBot.getOTP'(
	GlobalVariable.MyEmail_Id,
	GlobalVariable.Email_Key
)
...
otp1.eachWithIndex { String digit, int i ->
	WebUI.sendKeys(findTestObject("Appointment Booking/Chat Bot Appt Book/input_otp-${i}"), digit)
}
WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/button_NEXT'))
KeywordUtil.logInfo("STEP 6: OTP '${otp1}' entered and submitted")
