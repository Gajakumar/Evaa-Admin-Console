...
// ---- "No Slots" page copy (reschedule flow) ----
@Field String expectedHeadingText       = 'Choose Appointment Date'
@Field String expectedNoSlotsMessage    = 'We\'re sorry, but no appointment slots are currently available. ' +
		'Please use the back button to modify your search, or call us at +1-232-333-3333 so we can assist you further.'
@Field String expectedLocationLabel     = 'Location*'
...
/**
 * Verifies the "No appointment slots available" page shown during reschedule:
 * heading, message, Back/Exit buttons, then walks Back (to the search form)
 * and Exit (back to the "no slots" message, then through to the reschedule
 * greeting) - mirroring the exact click sequence of the original test.
 */
def verifyNoSlotsFlow() {
	// Resolve each test object once and reuse it, instead of calling
	// findTestObject() again every time the same element is used.
	TestObject noSlotsHeading     = findTestObject('Appointment Booking/Reschedule Appt/No Slots/h2_Choose Appointment Date')
	TestObject noSlotsMessage     = findTestObject('Appointment Booking/Reschedule Appt/No Slots/p_Were sorry, but no appointment slots are curr')
	TestObject backButton         = findTestObject('Appointment Booking/Reschedule Appt/No Slots/button_BACK (1)')
	TestObject exitButton         = findTestObject('Appointment Booking/Reschedule Appt/No Slots/button_EXIT')
	TestObject locationLabel      = findTestObject('Appointment Booking/Reschedule Appt/No Slots/label_Location (1)')
	TestObject locationSelect     = findTestObject('Appointment Booking/Reschedule Appt/No Slots/select_Location (1)')
	...
	KeywordUtil.logInfo('Verifying "No Slots" page heading text is displayed correctly')
	WebUI.assertElementText(noSlotsHeading, expectedHeadingText, NO_WAIT)

	KeywordUtil.logInfo('Verifying the "no appointment slots available" message text')
	WebUI.assertElementText(noSlotsMessage, expectedNoSlotsMessage, NO_WAIT)

	KeywordUtil.logInfo('Verifying BACK button is present on the page')
	WebUI.assertElementPresent(backButton, NO_WAIT)

	KeywordUtil.logInfo('Verifying EXIT button is present on the page')
	WebUI.assertElementPresent(exitButton, NO_WAIT)

	KeywordUtil.logInfo('Clicking BACK button to return to the search form')
	WebUI.click(backButton)

	KeywordUtil.logInfo('Verifying "Location*" label is displayed on the search form')
	WebUI.assertElementText(locationLabel, expectedLocationLabel, NO_WAIT)

	KeywordUtil.logInfo('Verifying Location dropdown is present on the search form')
	WebUI.assertElementPresent(locationSelect, NO_WAIT)

	KeywordUtil.logInfo('Clicking Next button')
	WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/button_NEXT'))
	WebUI.waitForElementNotVisible(loadingIcon, PAGE_TIMEOUT)

	KeywordUtil.logInfo('Re-verifying the "no appointment slots available" message text')
	WebUI.assertElementText(noSlotsMessage, expectedNoSlotsMessage, NO_WAIT)

	KeywordUtil.logInfo('Clicking EXIT button again to proceed')
...
selectLocationProviderReason(location, provider, noSlotReason)
WebUI.waitForElementNotVisible(loadingIcon, PAGE_TIMEOUT)

// STEPS 6-16: No appointment slots are available for the reselected criteria -
// verify the "No Slots" page content and the Back/Exit navigation from it
KeywordUtil.logInfo('Step 6-16: Verifying "No Slots Available" page and navigation')
verifyNoSlotsFlow()

KeywordUtil.logInfo('Part 3 complete: Reschedule (No Slots) flow validated successfully')

/* =============================================================================
 * PART 4: RE-VERIFY IN MAXIMEYES THAT THE ORIGINAL APPOINTMENT IS UNCHANGED
 * (Since the reschedule attempt in Part 3 hit "No Slots" and never completed,
 *  the original appointment booked in Part 1 must still be intact - same
 *  date/time/reason as before, not the reschedule values.)
 * ========================================================================== */

// STEP 1: Switch to the already-open, already-logged-in MaximEyes window
// (no need to log in again - re-using the session from Part 2 keeps this fast
//  and avoids opening a second, conflicting MaximEyes session)
KeywordUtil.logInfo('Step 1: Switching to the existing MaximEyes window')
WebUI.switchToWindowIndex(1)

// STEP 2-3: Re-search for the patient and confirm the appointment slot
// still shows the ORIGINAL date/time/reason (i.e. nothing changed)
KeywordUtil.logInfo('Step 2-3: Re-verifying original appointment slot is unchanged')
searchPatientAndVerifyAppointment(firstName, lastName, expectedDateTimeReason)

// STEP 4: Re-check the scheduler/calendar view for the same reason
KeywordUtil.logInfo('Step 4: Re-verifying appointment on the scheduler view is unchanged')
verifyAppointmentOnScheduler(expectedSpanText)
...
KeywordUtil.markPassed('Reschedule Appointment - No Slots test case passed successfully (original appointment unchanged)')
