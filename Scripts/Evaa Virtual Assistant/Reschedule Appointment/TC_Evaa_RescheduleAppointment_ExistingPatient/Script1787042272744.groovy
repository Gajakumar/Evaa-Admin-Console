...
String lastName    = 'Wiggum'
String patientAge  = '50yo'
String dob         = '02/29/2004'
...
// Build the URL as a real String first — avoids GString/overload ambiguity
String targetUrl = GlobalVariable.MaxUrlQA5
KeywordUtil.logInfo('Opening URL: ' + targetUrl)

List<Object> jsArgs = new ArrayList<Object>()
WebUI.executeJavaScript("window.open(arguments[0], '_blank');", [targetUrl])
...
// ---- Expected text/content values ----
String appointmentReasonText      = reason+ "|" +providerFirstName
String appointmentDateTimeText    = tomorrowFullDate+ "@" +apptTime
String appointmentStatusText      = 'CONFIRMED'
...
String tcAppointmentCard          = 'Appointment Booking/Reschedule Appt/div_Katalon Reason _ Katalon08_12_2026  12_00 A'
String tcAppointmentHeader        = 'Appointment Booking/Reschedule Appt/h6_Katalon Reason _ Katalon'
String tcAppointmentDateTime      = 'Appointment Booking/Reschedule Appt/p_08_12_2026  12_00 AM'
String tcAppointmentStatus        = 'Appointment Booking/Reschedule Appt/p_CONFIRMED'
String tcActionButton             = 'Appointment Booking/Reschedule Appt/button_Confirm _ Cancel _ Reschedule'
String tcAlreadyConfirmedMsg      = 'Appointment Booking/Reschedule Appt/p_This appointment is already confirmed'
String tcAppointmentInputRow      = 'Appointment Booking/Reschedule Appt/input_Katalon Reason _ Katalon08_12_2026  12_00'
String tcRescheduleButton         = 'Appointment Booking/Reschedule Appt/button_Reschedule'
String tcDescribeSymptomsTextarea = 'Appointment Booking/Reschedule Appt/textarea_Describe your symptoms or reason for th'
String tcNextButton               = 'Appointment Booking/Reschedule Appt/button_NEXT'
String expectedRescheduleMsg    = 'Your appointment has been rescheduled'
 
// Timeout (0 = use default/global timeout) - extracted for reuse
...
// Step 1: Verify the appointment card shows the combined reason, date/time,
// and status text as a single block
//KeywordUtil.logInfo('Step 1: Verifying appointment card combined text (reason + date/time + status)')
//WebUI.assertElementText(findTestObject(tcAppointmentCard), appointmentCombinedText, defaultTimeout)
//KeywordUtil.logInfo('Step 1 passed: Appointment card text matches expected value')
// 
//// Step 2: Verify the appointment header text (reason line) is correct
//KeywordUtil.logInfo('Step 2: Verifying appointment header text')
//WebUI.assertElementText(findTestObject(tcAppointmentHeader), appointmentReasonText, defaultTimeout)
//KeywordUtil.logInfo('Step 2 passed: Appointment header text matches expected value')
// 
//// Step 3: Verify the appointment date/time text is correct
//KeywordUtil.logInfo('Step 3: Verifying appointment date/time text')
//WebUI.assertElementText(findTestObject(tcAppointmentDateTime), appointmentDateTimeText, defaultTimeout)
//KeywordUtil.logInfo('Step 3 passed: Appointment date/time text matches expected value')
// 
//// Step 4: Verify the appointment status text is correct
//KeywordUtil.logInfo('Step 4: Verifying appointment status text')
//WebUI.assertElementText(findTestObject(tcAppointmentStatus), appointmentStatusText, defaultTimeout)
//KeywordUtil.logInfo('Step 4 passed: Appointment status text matches expected value')
...
WebUI.click(findTestObject(tcActionButton))
KeywordUtil.logInfo('Step 5 passed: Action button clicked')
 
// Step 6: Verify the "already confirmed" warning message is displayed
KeywordUtil.logInfo('Step 6: Verifying "already confirmed" message is displayed')
WebUI.assertElementText(findTestObject(tcAlreadyConfirmedMsg), alreadyConfirmedMessage, defaultTimeout)
KeywordUtil.logInfo('Step 6 passed: "Already confirmed" message displayed as expected')
 
//// Step 7: Re-verify appointment header text is still correct after the click
//KeywordUtil.logInfo('Step 7: Re-verifying appointment header text after action button click')
//WebUI.assertElementText(findTestObject(tcAppointmentHeader), appointmentReasonText, defaultTimeout)
//KeywordUtil.logInfo('Step 7 passed: Appointment header text still matches expected value')
// 
//// Step 8: Re-verify appointment date/time text is still correct
//KeywordUtil.logInfo('Step 8: Re-verifying appointment date/time text after action button click')
//WebUI.assertElementText(findTestObject(tcAppointmentDateTime), appointmentDateTimeText, defaultTimeout)
//KeywordUtil.logInfo('Step 8 passed: Appointment date/time text still matches expected value')
// 
//// Step 9: Re-verify appointment status text is still correct
//KeywordUtil.logInfo('Step 9: Re-verifying appointment status text after action button click')
//WebUI.assertElementText(findTestObject(tcAppointmentStatus), appointmentStatusText, defaultTimeout)
//KeywordUtil.logInfo('Step 9 passed: Appointment status text still matches expected value')
// 
//// Step 10: Assert the appointment input row is present on the page
//KeywordUtil.logInfo('Step 10: Verifying appointment input row is present')
//WebUI.assertElementPresent(findTestObject(tcAppointmentInputRow), defaultTimeout)
//KeywordUtil.logInfo('Step 10 passed: Appointment input row is present')
// 
//// Step 11: Click the appointment input row to select it
//KeywordUtil.logInfo('Step 11: Clicking the appointment input row')
//WebUI.click(findTestObject(tcAppointmentInputRow))
//KeywordUtil.logInfo('Step 11 passed: Appointment input row clicked')
 
// Step 12: Click the "Reschedule" button
KeywordUtil.logInfo('Step 12: Clicking the Reschedule button')
WebUI.click(findTestObject(tcRescheduleButton))
KeywordUtil.logInfo('Step 12 passed: Reschedule button clicked')
...
// STEP 9: Select appointment time
KeywordUtil.logInfo("Step 9: Selecting appointment time - ${rescheduleApptTime}")
WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/button_10_30 AM', ['time': rescheduleApptTime]))
WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/button_NEXT'))

// STEP 10: Enter reason/symptoms free-text
KeywordUtil.logInfo("Step 10: Entering reason for visit - '${rescheduleReasonInput}'")
WebUI.setText(findTestObject('Appointment Booking/Chat Bot Appt Book/textarea_Describe your symptoms or reason for th'), rescheduleReasonInput)
WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/button_NEXT'))

// STEP 11: Select self pay option
KeywordUtil.logInfo('Step 11: Selecting "Self Pay / No Insurance Available"')
WebUI.click(findTestObject('Appointment Booking/Chat Bot Appt Book/input_Self Pay_No Insurance Available'))
...
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Your appointment has been rescheduled'), expectedRescheduleMsg, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Name_ Marge Simpson'), expectedName, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Location_ Katalon Location'), expectedLocation, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Provider_ Katalon Provider (OD)'), expectedProvider, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Reason_ Katalon Reason'), expectedReason, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Date_ 08_11_2026', ['date': tomorrowFullDate]), expectedDate, SHORT_TIMEOUT)
WebUI.assertElementText(findTestObject('Appointment Booking/Reschedule Appt/p_Time_ 04_00 PM', ['time': rescheduleApptTime]), expectedReschTime, SHORT_TIMEOUT)
...
String expectedReschDateTimeReason   = "${tomorrowFullDate} | ${rescheduleApptTime} | ${reason}"
...
// STEP 3: Verify appointment slot details on the search/results view
KeywordUtil.logInfo("Step 3: Verifying appointment slot shows '${expectedReschDateTimeReason}'")
...
WebUI.assertElementText(
	findTestObject('MaximeyesAppt/Page_MaximEyes/div_07_23_2026 _ 09_30 AM _ Katalon Reason'),
	expectedReschDateTimeReason,
	SHORT_TIMEOUT)

String actualReschLocationOV = WebUI.getText(findTestObject('MaximeyesAppt/Page_MaximEyes/div_Katalon, Katalon Location')).trim()
WebUI.verifyEqual(actualReschLocationOV, expectedPatientLocation)
...
WebUI.comment('STEP 30: Verify patient/location span text matches "Katalon, Katalon Location"')
String actualReschLocation = WebUI.getText(
	findTestObject('MaximeyesAppt/Page_MaximEyes/span_Katalon, Katalon Location')
).trim()

WebUI.verifyEqual(actualReschLocation, expectedPatientLocation)
